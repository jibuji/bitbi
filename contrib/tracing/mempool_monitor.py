...
""" Example logging Bitbi Core mempool events using the mempool:added,
...
def main(bitbid_path):
    bitbid_with_usdts = USDT(path=str(bitbid_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bitbid_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bitbid_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bitbid_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bitbid_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bitbid_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bitbid")
        exit(1)
