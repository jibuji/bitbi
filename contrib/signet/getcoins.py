#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Bitbi Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitbi-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitbi-cli', help='bitbi-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bitbi address to which the faucet should send')
...
parser.add_argument('bitbi_cli_args', nargs='*', help='Arguments to pass on to bitbi-cli (default: -signet)')

args = parser.parse_args()

if args.bitbi_cli_args == []:
    args.bitbi_cli_args = ['-signet']


def bitbi_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitbi_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitbi_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitbi_cli(['getnewaddress', 'faucet', 'bech32'])
