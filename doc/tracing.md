# User-space, Statically Defined Tracing (USDT) for Bitbi Core

Bitbi Core includes statically defined tracepoints to allow for more
...
**Note**: Bitbi Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bitbi Core
...
Multiple tools can list the available tracepoints in a `bitbid` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bitbi Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bitbid
stap net        outbound_message 0x0000000000107c05 /src/bitbid
stap validation block_connected  0x00000000002fb10c /src/bitbid
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bitbi Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitbid | grep NT_STAPSDT -A 4 -B 2
