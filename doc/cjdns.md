# CJDNS support in Bitbi Core

It is possible to run Bitbi Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bitbi Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bitbi network and individual nodes.
...
before it will work with your Bitbi Core node. This is described in steps
...
## Run Bitbi Core with CJDNS

Once you are connected to the CJDNS network, the following Bitbi Core
...
When enabled, this option tells Bitbi Core that it is running in an
...
IPv6 local network. This helps Bitbi Core perform better address management:
...
CJDNS support was added to Bitbi Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitbi-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bitbi/bitbi/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bitbi Core

There are several ways to see your CJDNS address in Bitbi Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitbi-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitbi-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitbi-cli help getnodeaddresses` for details.
