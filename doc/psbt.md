# PSBT Howto for Bitbi Core

Since Bitbi Core 0.17, an RPC interface exists for Partially Signed Bitbi
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitbi/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Bitbi transactions that are not fully signed
...
#### Multisig with multiple Bitbi Core instances
...
Bitbi Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitbi-cli` is used.
...
Later, when *V* BTB has been received on *Amulti*, and Bob and Carol want to
