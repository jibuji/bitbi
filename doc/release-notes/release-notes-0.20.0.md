...
Bitbi Core version 0.20.0 is now available from:

  <https://bitbicore.org/bin/bitbi-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Bitbi Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Bitbi Core version 0.18.0.
...
- Testing or debugging of implementations of the Bitbi P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Bitbi Core. Bitbi Core logs debug messages
...
- OpenSSL is no longer used by Bitbi Core.  (#17265)

- BIP70 support has been fully removed from Bitbi Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitbi/bitbi/issues/16599),
  [PR #16702](https://github.com/bitbi/bitbi/pull/16702), and the
  `bitbid help` for more information.  This option is experimental and
...
- All custom settings configured when Bitbi Core starts are now
...
- The "Start Bitbi Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitbicore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Bitbi Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Bitbi Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitbi-core/HWI) for
  signing.  Future versions of Bitbi Core should support a GUI option
...
- Bitbi Core's automatically-generated source code documentation is
  now available at https://doxygen.bitbicore.org.  (#17596)
...
- The `bitbi-cli` utility used with the `-getinfo` parameter now
...
- The `bitbi-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=9801` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `bitbistrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitbi-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_bitbi` binary (hebasto)
...
- #18702 Fix ASLR for bitbi-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitbi schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitbi.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitbi` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitbi-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitbi-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Bitbi Core files (hebasto)
...
- #17119 Fix broken bitbi-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitbi-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitbicore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitbi-wallet (hel-o)
...
- #18341 Replace remaining literal BTB with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitbid startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitbi-cli -getinfo (malevolent)
