...
class ConfArgsTest(BitbiTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in bitbi.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "bitbi_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'bitbi.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "bitbi_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # Write a bitbi.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.bitbiconf.read_text()
        conf_path = default_datadir / "bitbi.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a bitbi.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitbi.conf" file which is ignored, because a '
        ...
        # the same bitbi.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/bitbi.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when bitbi.conf in the default data directory sets another datadir '
                      'and it contains a different bitbi.conf file that would be ignored')
        ...
        # bitbi.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "bitbi.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "bitbi.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/bitbi.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "bitbi.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "bitbi.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_bitbi.conf"
