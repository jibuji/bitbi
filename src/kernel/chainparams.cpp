...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward, const Consensus::Params& consensus)
{
    ...
    assert(!fNegative && !fOverflow && bnTarget != 0);
    
    CMutableTransaction txNew;
    txNew.nVersion = 2;
    txNew.vin.resize(1);
    txNew.vout.resize(1);
    txNew.vin[0].scriptSig = CScript() << 0x1e3fffff << CScriptNum(4) << std::vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
{
    const char* pszTimestamp = "Spot Bitcoin ETF received official approval from the SEC on January 10, 2024.";
    std::vector<unsigned char> data = ParseHex(GenesisRewardReceiverAddr);
    CScript genesisOutputScript(data.begin(), data.end());
...
public:
    CMainParams() {
        ...
        consensus.nPowTargetSpacing = 5 * 60; // 5 minutes per block
        ...
        consensus.nMinimumChainWork = uint256{};//uint256S("0x000000000000000000000000000000000000000052b2559353df4117b7348b64");
        consensus.defaultAssumeValid = uint256{};//uint256S("0x00000000000000000001a0a448d6cf2546b06801389cc030b2b18c6491266815"); // 804000
        ...
        genesis = CreateGenesisBlock(1709966000, 42511, 0x1e3fffff, 4, INITIAL_REWARD, consensus);
        ...
        vSeeds.emplace_back("dnsseed1.bitwealth.network.");
...
public:
    CTestNetParams() {
        ...
        // genesis = CreateGenesisBlock(1296688602, 414098458, 0x1d00ffff, 1, INITIAL_REWARD, consensus);
        // consensus.hashGenesisBlock = genesis.GetHash();
...
public:
    explicit SigNetParams(const SigNetOptions& options)
    {
        ...
        // genesis = CreateGenesisBlock(1598918400, 52613770, 0x1e0377ae, 1, INITIAL_REWARD, consensus);
        // consensus.hashGenesisBlock = genesis.GetHash();
...
public:
    explicit CRegTestParams(const RegTestOptions& opts)
    {
        ...
        // genesis = CreateGenesisBlock(1296688602, 2, 0x207fffff, 1, INITIAL_REWARD, consensus);
        // consensus.hashGenesisBlock = genesis.GetHash();
