...
/** Call node.updateRwSetting() with Bitbi 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Bitbi 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bitbi/bitbi/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitbi 22.x will not try to read these.
...
//! Convert enabled/size values to bitbi -prune setting.
...
//! Get pruning enabled value to show in GUI from bitbi -prune setting.
...
//! Get pruning size value to show in GUI from bitbi -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitbiUnit")) {
        settings.setValue("DisplayBitbiUnit", QVariant::fromValue(BitbiUnit::BTB));
    }
    QVariant unit = settings.value("DisplayBitbiUnit");
    if (unit.canConvert<BitbiUnit>()) {
        m_display_bitbi_unit = unit.value<BitbiUnit>();
    } else {
        m_display_bitbi_unit = BitbiUnit::BTB;
        settings.setValue("DisplayBitbiUnit", QVariant::fromValue(m_display_bitbi_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitbi.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitbi_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitbiUnit>() == m_display_bitbi_unit) return;
    m_display_bitbi_unit = new_unit.value<BitbiUnit>();
    QSettings settings;
    settings.setValue("DisplayBitbiUnit", QVariant::fromValue(m_display_bitbi_unit));
    Q_EMIT displayUnitChanged(m_display_bitbi_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bitbi/bitbi/pull/8273
    ...
    // (https://github.com/bitbi-core/gui/issues/567).
