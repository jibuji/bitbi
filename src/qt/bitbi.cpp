...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitbiUnit>("BitbiUnit");
#else
    qRegisterMetaType<BitbiUnit>("BitbiUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitbi_de.qm (shortcut "de" needs to be defined in bitbi.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitbi_de_DE.qm (shortcut "de_DE" needs to be defined in bitbi.qrc)
...
static const char* qt_argv = "bitbi-qt";

BitbiApplication::BitbiApplication()
...
void BitbiApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitbiApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitbiGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitbiApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitbiApplication::createOptionsModel(bool resetSettings)
{
...
void BitbiApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitbiGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitbiGUI::quitRequested, this, &BitbiApplication::requestShutdown);
...
void BitbiApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitbiApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitbiApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitbiApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitbiApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitbiApplication::handleRunawayException);
    connect(this, &BitbiApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitbiApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitbiApplication::parameterSetup()
{
...
void BitbiApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitbiApplication::requestInitialize()
{
...
void BitbiApplication::requestShutdown()
{
...
void BitbiApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bitbi: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitbiGUI::handlePaymentRequest);
            connect(window, &BitbiGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitbiApplication::handleRunawayException(const QString &message)
{
...
void BitbiApplication::handleNonFatalException(const QString& message)
{
...
WId BitbiApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitbiApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitbiGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitbi);
    Q_INIT_RESOURCE(bitbi_locale);
    ...
    /// 6-7. Parse bitbi.conf, determine network, switch to network specific
    ...
    // bitbi: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
