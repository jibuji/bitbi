...
const char * const BITCOIN_CONF_FILENAME = "bitbi.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitbi-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bitbi
    // macOS: ~/Library/Application Support/Bitbi
    // Unix-like: ~/.bitbi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitbi";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bitbi";
#else
    // Unix-like
    return pathRet / ".bitbi";
#endif
#endif
}
