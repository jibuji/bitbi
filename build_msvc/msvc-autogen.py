...
def parse_config_into_btb_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Bitbi Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitbi_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/bitbi_config.h'), "w", encoding="utf8") as btb_config:
        btb_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Bitbi-core msbuild configuration initialiser.')
    ...
    parse_config_into_btb_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/bitbi_config.h'), os.path.join(SOURCE_DIR, 'config/bitbi-config.h'))
